...
double GetTradeVolume(string pair, double stoploss)
{
  ...
  double lot = account.Balance() * RiskPerTrade *
               symbol.TickSize() *
               symbol.TickValue() /
               stoploss;
...
double GetMaxLossInPoint(string pair = "")
{
  ...
  return account.Balance() * RiskPerTrade *
         symbol.TickSize() /
         symbol.TickValue() /
         trade_volume;
}
...
{
  ENUM_POSITION_TYPE position_type = position.PositionType();
  double open_price = position.PriceOpen();
  double current_stop_loss = position.StopLoss();
  double current_take_profit = position.TakeProfit();
